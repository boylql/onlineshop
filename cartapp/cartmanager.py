...
class SessionCartManager(CartManager):
    ...
    def __get_key(self, goodsid):
        return goodsid

    def add(self, goodsid, count, *args, **kwargs):

        # 获取购物项的唯一标示
        key = self.__get_key(goodsid)

        # session   {'cart':{key1:item}}

        # session('cart',[{key1:cartitem,key2:cartitem}])
        if key in self.session[self.cart_name]:
            self.update(goodsid, count, *args, **kwargs)
        else:
            self.session[self.cart_name][key] = jsonpickle.dumps(
                CartItem(goodsid=goodsid, count=count))

    def delete(self, goodsid, *args, **kwargs):
        key = self.__get_key(goodsid)
        if key in self.session[self.cart_name]:
            del self.session[self.cart_name][key]

    def update(self, goodsid, step, *args, **kwargs):
        key = self.__get_key(goodsid)
        if key in self.session[self.cart_name]:
...
class DBCartManger(CartManager):
    ...
    def add(self, goodsid, count, *args, **kwargs):
        ...
        if self.user.cartitem_set.filter(goodsid=goodsid).count() == 1:

            self.update(goodsid, count, *args, **kwargs)
        else:
            CartItem.objects.create(goodsid=goodsid, count=count, user=self.user)

    def delete(self, goodsid, *args, **kwargs):
        self.user.cartitem_set.filter(goodsid=goodsid).update(count=0, isdelete=True)

    def update(self, goodsid, step, *args, **kwargs):

        self.user.cartitem_set.filter(goodsid=goodsid).update(
    ...
    def get_cartitems(self, goodsid, *args, **kwargs):
        return self.user.cartitem_set.get(goodsid=goodsid)
